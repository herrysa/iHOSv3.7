...
public class ExportHelper {
    public static ColumnDef[] getColumnDefs( JdbcTemplate jdbcTemplate, String paramString ) {
        ...
        for ( int i = 0; i < localResultSetMetaData.getColumnCount(); i++ )
            arrayOfColumnDef1[i] =
                new ColumnDef( localResultSetMetaData.getColumnName( i + 1 ), localResultSetMetaData.getColumnType( i + 1 ), null, null );
    ...
    private static int paramType( SearchOption ops ) {
        int type = 8;
        if ( ops.getFieldType().equals( "String" ) )
            type = 1;

        else if ( ops.getFieldType().equals( "Integer" ) )
            type = 2;

        else if ( ops.getFieldType().equals( "Number" ) )
            type = 3;

        else if ( ops.getFieldType().equals( "Percent" ) )
            type = 4;

        else if ( ops.getFieldType().equals( "Boolean" ) )
            type = 5;

        else if ( ops.getFieldType().equals( "Money" ) )
            type = 6;

        else if ( ops.getFieldType().equals( "Date" ) )
            type = 7;
