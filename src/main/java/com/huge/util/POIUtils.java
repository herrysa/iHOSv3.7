...
//		toStyle.setFont(fromStyle.getFont(null));
	...
	public static void copySheet(HSSFWorkbook wb,HSSFSheet fromSheet, HSSFSheet toSheet,
		...
		//合并区域处理
		mergerRegion(fromSheet, toSheet);Region region = null;
	...
	public static void copyRow(HSSFWorkbook wb,HSSFRow fromRow,HSSFRow toRow,boolean copyValueFlag){
		...
		Sheet toSheet = toRow.getSheet();
				for (int j = fromRow.getLastCellNum(); j >= fromRow.getFirstCellNum(); j--) {
					...
					if (colnum > 100) {
						toSheet.setColumnWidth((short) j, (short) colnum);
					}
					if (colnum == 0) {
						toSheet.setColumnHidden((short) j, true);
					} else {
						toSheet.setColumnHidden((short) j, false);
					}
				}
	...
	*/
	public static void mergerRegion(HSSFSheet fromSheet, HSSFSheet toSheet) {
	   ...
	   for (int i = 0; i < sheetMergerCount; i++) {
	    ...
	    if ((mergedRegionAt.getRowFrom() >= fromSheet.getFirstRowNum())&& (mergedRegionAt.getRowTo() <= fromSheet.getLastRowNum())) {
