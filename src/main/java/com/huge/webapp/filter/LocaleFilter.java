package com.huge.webapp.filter;
...
public class LocaleFilter
    ...
    public void doFilterInternal( HttpServletRequest request, HttpServletResponse response, FilterChain chain )
        ...
        String subSystem = request.getParameter( "currentSubSystem" );
        /*	if (subSystem != null&&!subSystem.trim().equalsIgnoreCase("")&&!subSystem.trim().equalsIgnoreCase("MainMenu")) {
        		Map<String, Object> config = (Map) getServletContext().getAttribute(Constants.CONFIG);
        		config.put(Constants.SUB_SYSTEM, subSystem);
        	}else{
        		Map<String, Object> config = (Map) getServletContext().getAttribute(Constants.CONFIG);
        		config.put(Constants.SUB_SYSTEM, "MainMenu");
        	}*/
        if ( subSystem != null && !subSystem.trim().equalsIgnoreCase( "" ) ) {
            Map<String, Object> config = (Map) getServletContext().getAttribute( Constants.CONFIG );
            config.put( Constants.SUB_SYSTEM, subSystem );
        }/*else{
         Map<String, Object> config = (Map) getServletContext().getAttribute(Constants.CONFIG);
