				...
				addUrlData = function() {
					...
					jQuery.ajax({
						...
						success: function(data) {
							var gridDatas = $.zgrid.getAccessor(data,datas);
							var dataSource = ts.grid.dataSource = [];
							for(var rowIndex in gridDatas){
								var rowData = gridDatas[rowIndex];
								rowIndex = parseInt(rowIndex);
								dataSource[rowIndex] = {};
								for(var modelIndex in initTableColModels){
									var v = $.zgrid.getAccessor(rowData,initTableColModels[modelIndex].name);
									var colName = initTableColModels[modelIndex].name;
									dataSource[rowIndex][colName] = v ;
								}
							}
	...
	$.zgrid.extend({
		...
		fillData: function(){
			...
			for(var rowDataIndex in dataSource){
				...
				jQuery(this).addRowData(rowDataIndex,rowData);
			}
			this.footData();
		},
		rowChange: function(rowid,rowData){
			...
			if(rowid){
				...
				sn = parseInt(sn);
				var dataSource = $t.grid.dataSource;
				var colModes = $t.p.colModel;
				if(!dataSource[sn]){
					dataSource[sn] = {};
				}
				for(var modelIndex in colModes){
					var v = $.zgrid.getAccessor(rowData,colModes[modelIndex].name);
					dataSource[sn][colModes[modelIndex].name] = v ;
				}
		...
		addGridRowData: function(rowid, rowdata) {
			var $t = this[0];
			if (!$t || !$t.grid) {
				return;
			}
		...
		saveGrid: function() {
			...
			for(var rowIndex in dataSource){
				if(!jQuery.isEmptyObject(dataSource[rowIndex])){
		...
		fullEdit: function() {
			...
			if ($t.p.cellEdit == true) {
				...
				jQuery("#" + $t.id).on("keydown","input[type=text]",function(event){
					event.stopPropagation();
					var activeCell = $t.p.activeCell;
					if(activeCell.state==1){
					 var keyCode = event.keyCode;
...
/*
		...
		addLocalData: function(rowDatas) {
			...
			for(var rowIndex in gridDatas){
				var rowData = gridDatas[rowIndex];
				var rowSn = parseInt(rowIndex) + dataLength;
				dataSource[rowSn] = {};
				for(var modelIndex in colModels){
					var v = $.zgrid.getAccessor(rowData,colModels[modelIndex].name);
					dataSource[rowSn][colModels[modelIndex].name] = v ;
				}
			}
