...
TabPanel.prototype = {
  ...
  show : function(position, notExecuteMoveSee){
    if(this.tabs.length<1)
      return false;
    var oldPosition = this.getActiveIndex();
    if((oldPosition!=position)&&this.tabs[oldPosition]){
    	this.tabs[oldPosition].content.addClass('display_none');
    }
    position = this.getTabPosision(position);
    if(typeof position == 'string')
      position = 0;
    if(this.scrollFinish)
    {
      ...
      else
      {
        ...
        this.tabs[position].content.removeClass('display_none');
        if(!notExecuteMoveSee)
        {
          this.moveToSee(position);
        }
  ...
  /**
   * @description {Method} getTabs To get tabItems are in the panel.
