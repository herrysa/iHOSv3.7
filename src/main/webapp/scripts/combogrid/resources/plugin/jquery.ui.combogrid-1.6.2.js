...
$.widget( "cg.combogrid", {
	options: {
		...
		okIcon:false,
		ifr:false,
		combogridId:"combogrid_menucombo",
		alternate: false,
		appendTo: "body",
		autoFocus: false,
		autoChoose : false,
	...
	_create: function() {
		...
		//ESCAPE needs this workaround
			...
			this.options.source =  function( request, response ) {
				...
				var thisQueryParams = self.options.queryParams;
				for(var item in thisQueryParams){
					ajaxData[item] = thisQueryParams[item];
				}
				$.ajax({
		...
		this.menucombo = $( "<div id='"+self.options.combogridId+"'></div>" )
			.addClass( "cg-autocomplete" )
			.resize(function(e,w,h,t,l,hide){
					if(self.options.ifr){
						bgiframe($("#"+self.options.combogridId),e,w,h,t,l,hide);
					}
		    	})
...
// Select page
	...
	_move: function( direction, event ) {
		var beforeMove = this.options.beforeMove;
