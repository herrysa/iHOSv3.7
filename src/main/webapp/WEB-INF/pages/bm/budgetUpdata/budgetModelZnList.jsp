...
<script type="text/javascript">
	var budgetModelZnLayout;
	var budgetModelZnGridIdString="#budgetModelZn_gridtable";
    	...
    	budgetModelZnGrid.jqGrid({
			...
			onSelectRow: function(rowid) {
		 	...
		 	gridComplete:function(){
           	...
           	// }
           	gridContainerResize('budgetModelZn','div');
           	var dataTest = {"id":"budgetModelZn_gridtable"};
    ...
    jQuery(budgetModelZnGrid).jqGrid('bindKeys');
    ...
    jQuery("#budgetModelZn_gridtable_xf").click(function(){
    	...
    	$.get("budgetModel_Xf", {
			"_" : $.now(),xfId:sid,navTabId:'budgetModelZn_gridtable'
		}, function(data) {
			formCallBack(data);
		});
    });
    jQuery("#budgetModelZn_gridtable_reXf").click(function(){
    	var sid = jQuery("#budgetModelZn_gridtable").jqGrid('getGridParam','selarrrow');
    	if(sid.length==0){
    		alertMsg.error("请选择记录！");
    		return ;
    	...
    	alertMsg.confirm("确认重新汇总？重新汇总后,之前汇总的数据将作废！", {
			okCall: function(){
				jQuery.post("budgetModel_Xf", {
					"_" : $.now(),xfId:sid,xfType:'4',navTabId:'budgetModelZn_gridtable'
				}, function(data) {
					formCallBack(data);
				},"json");
    ...
    jQuery("#budgetModelZn_gridtable_comfirm").click(function(){
    	var sid = jQuery("#budgetModelZn_gridtable").jqGrid('getGridParam','selarrrow');
    	if(sid.length==0){
    		alertMsg.error("请选择记录！");
    		return ;
    	...
    	var updataId = rowData['updataId'];
    	$.get("confirmBmUpdata", {
			"_" : $.now(),updataId:updataId,navTabId:'budgetModelZn_gridtable'
		}, function(data) {
			formCallBack(data);
		});
    });
    
    jQuery("#budgetModelZn_gridtable_del_c").click(function(){
    	var sid = jQuery("#budgetModelZn_gridtable").jqGrid('getGridParam','selarrrow');
    	if(sid.length==0){
    		alertMsg.error("请选择记录！");
    		return ;
    	...
    	for(var i in sid){
    		var id = sid[i];
	    	var rowData = jQuery("#budgetModelZn_gridtable").jqGrid('getRowData',id);
    	...
    	alertMsg.confirm("确认删除？", {
			okCall: function(){
				jQuery.post("budgetModelXfDel", {
					"_" : $.now(),id:sid,xfType:2,navTabId:'budgetModelZn_gridtable'
				}, function(data) {
					formCallBack(data);
				},"json");
...
<div class="page">
<div id="budgetModelZn_container">
	<div id="budgetModelZn_layout-center" class="pane ui-layout-center">
	<div id="budgetModelZn_pageHeader" class="pageHeader">
			<div class="searchBar">
				<div class="searchContent">
				<form id="budgetModelZn_search_form" >
					...
					<div class="buttonActive" style="float:right">
						<div class="buttonContent">
							<button type="button" onclick="propertyFilterSearch('budgetModelZn_search_form','budgetModelZn_gridtable')"><s:text name='button.search'/></button>
						</div>
					</div>
				</form>
				</div>
	...
	<div class="pageContent">
		<div id="budgetModelZn_buttonBar" class="panelBar">
			<ul class="toolBar">
				<li><a id="budgetModelZn_gridtable_refresh" class="initbutton" href="javaScript:" ><span>刷新
					</span>
				</a>
				</li>
				...
				<s:if test="needBmZnCheckProcess!='1'">
				<li><a id="budgetModelZn_gridtable_comfirm" class="submitbutton" href="javaScript:" ><span>提交审批</span></a></li>
				</s:if>
				<li><a id="budgetModelZn_gridtable_del_c" class="delbutton"  href="javaScript:"
					><span>删除
					</span>
				</a>
				</li>
		...
		<div id="budgetModelZn_gridtable_div" class="grid-wrapdiv" buttonBar="width:500;height:300">
			<input type="hidden" id="budgetModelZn_gridtable_navTabId" value="${sessionScope.navTabId}">
			<label style="display: none" id="budgetModelZn_gridtable_addTile">
				<s:text name="budgetModelZnNew.title"/>
			</label> 
			<label style="display: none"
				id="budgetModelZn_gridtable_editTile">
				<s:text name="budgetModelZnEdit.title"/>
			</label>
			<div id="load_budgetModelZn_gridtable" class='loading ui-state-default ui-state-active' style="display:none"></div>
 			<table id="budgetModelZn_gridtable"></table>
			<!--<div id="budgetModelZnPager"></div>-->
		</div>
	</div>
	<div id="budgetModelZn_pageBar" class="panelBar">
		<div class="pages">
			<span><s:text name="pager.perPage" /></span> <select id="budgetModelZn_gridtable_numPerPage">
				<option value="20">20</option>
				<option value="50">50</option>
				<option value="100">100</option>
				<option value="200">200</option>
			</select> <span><s:text name="pager.items" />. <s:text name="pager.total" /><label id="budgetModelZn_gridtable_totals"></label><s:text name="pager.items" /></span>
		</div>
		<div id="budgetModelZn_gridtable_pagination" class="pagination"
...
<div id="budgetModelZn_layout-south" class="pane ui-layout-south" style="padding: 2px">
		<div class="panelBar">
						<ul class="toolBar">
							<li style="float: right;"><a id="budgetModelZn_close" class="closebutton"
							...
							<li style="float: right;"><a id="budgetModelZn_fold" class="foldbutton"
								href="javaScript:"><span><fmt:message
											key="button.fold" />
								</span>
							</a></li>
							<li class="line" style="float: right">line</li>
							<li style="float: right"><a id="budgetModelZn_unfold" class="unfoldbutton"
								href="javaScript:"><span><fmt:message
											key="button.unfold" />
								</span>
							</a></li>
