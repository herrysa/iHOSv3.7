...
<script type="text/javascript">
	var recruitResumeGridIdString="#recruitResumeTalentPool_gridtable";
	
	jQuery(document).ready(function() { 
		var initFlag = 0;
		var recruitResumeGrid = jQuery(recruitResumeGridIdString);
	    recruitResumeGrid.jqGrid({
	    	url : "recruitResumeGridList?filter_EQS_favoriteState=2",
	        ...
	        colModel:[
				...
				{name:'maritalStatus',index:'maritalStatus',width : 70,align:'left',label : '<s:text name="recruitResume.maritalStatus" />',hidden:false,highsearch:true},				
				...
				{name:'overseaAssignment',index:'overseaAssignment',width : 120,align:'center',label : '<s:text name="recruitResume.overseaAssignment" />',hidden:true,formatter:'checkbox',highsearch:true},				
...
// 				{name:'recruitDate',index:'recruitDate',width : 80,align:'center',label : '<s:text name="recruitResume.recruitDate" />',hidden:true,formatter:"date",formatoptions:{newformat : 'Y-m-d'},highsearch:true},				
...
// 				{name:'yearMonth',index:'yearMonth',align:'center',width : 50,label : '<s:text name="期间" />',hidden:false,highsearch:true},	
				...
				{name:'talentPoolDate',index:'talentPoolDate',width : 70,align:'center',label : '<s:text name="recruitResume.talentPoolDate" />',hidden:false,formatter:"date",formatoptions:{newformat : 'Y-m-d'},highsearch:true}	,			
	        ...
	        onSelectRow: function(rowid) {
		 	...
		 	gridComplete:function(){
				 ...
				 gridContainerResize('recruitResumeTalentPool','div');
	            ...
	            if(rowNum>0){
	                ...
	                for(var i=0;i<rowNum;i++){
		            	...
		            	var gridName="recruitResumeTalentPool_gridtable";
           		...
           		var dataTest = {"id":"recruitResumeTalentPool_gridtable"};
      	   		jQuery.publish("onLoadSelect",dataTest,null);
      	   	initFlag = initColumn('recruitResumeTalentPool_gridtable','com.huge.ihos.hr.recruitResume.model.RecruitResume',initFlag);
       		} 
    	});
    	jQuery(recruitResumeGrid).jqGrid('bindKeys');
    	//实例化ToolButtonBar
        var recruitResumeTalentPool_menuButtonArrJson = "${menuButtonArrJson}";
        var recruitResumeTalentPool_toolButtonCollection = new ToolButtonCollection(eval(zzhUtil.DecodeURI(recruitResumeTalentPool_menuButtonArrJson,false)));
        var recruitResumeTalentPool_toolButtonBar = new ToolButtonBar({el:$('#recruitResumeTalentPool_buttonBar'),collection:recruitResumeTalentPool_toolButtonCollection,attributes:{
         tableId : 'recruitResumeTalentPool_gridtable',
        ...
        //实例化结束
        var recruitResumeTalentPool_function = new scriptFunction();
        recruitResumeTalentPool_function.optBeforeCall = function(e,$this,param){
			var pass = false;
    ...
    //为button添加方法 (普通点击按钮)
    //删除
     recruitResumeTalentPool_toolButtonBar.addCallBody('1004030401',function(e,$this,param){
    	 var url = "${ctx}/recruitResumeGridEdit?oper=del";
      	var sid = jQuery("#recruitResumeTalentPool_gridtable").jqGrid('getGridParam','selarrrow');
      	if(sid==null|| sid.length == 0){       	
				...
				for(var i = 0;i < sid.length; i++){
					var rowId = sid[i];
					var row = jQuery("#recruitResumeTalentPool_gridtable").jqGrid('getRowData',rowId);
				...
				url = url+"&id="+sid+"&navTabId=recruitResumeTalentPool_gridtable";
     ...
     recruitResumeTalentPool_toolButtonBar.addBeforeCall('1004030401',function(e,$this,param){
			return recruitResumeTalentPool_function.optBeforeCall(e,$this,param);
 	},{checkPeriod:"checkPeriod"});
   //移出
     recruitResumeTalentPool_toolButtonBar.addCallBody('1004030402',function(e,$this,param){
     ...
     recruitResumeTalentPool_toolButtonBar.addBeforeCall('1004030402',function(e,$this,param){
			return recruitResumeTalentPool_function.optBeforeCall(e,$this,param);
	},{checkPeriod:"checkPeriod"});
   //审查合格
     recruitResumeTalentPool_toolButtonBar.addCallBody('1004030403',function(e,$this,param){
     ...
     recruitResumeTalentPool_toolButtonBar.addBeforeCall('1004030403',function(e,$this,param){
			return recruitResumeTalentPool_function.optBeforeCall(e,$this,param);
	},{checkPeriod:"checkPeriod"});
   //设置表格列
        var recruitResumeTalentPool_setColShowButton = {id:'1004030404',buttonLabel:'<s:text name="button.setColShow" />',className:"settlebutton",show:true,enable:true,
          callBody:function(){
           setColShow('recruitResumeTalentPool_gridtable','com.huge.ihos.hr.recruitResume.model.RecruitResume');
          }};
        recruitResumeTalentPool_toolButtonBar.addButton(recruitResumeTalentPool_setColShowButton);//实例化ToolButtonBar
  	});
	//状态超链接查看
	//报到方式
...
<div class="page">
	<div class="pageHeader" id="recruitResumeTalentPool_pageHeader">
		<div class="searchBar">
			<div class="searchContent">
				<form id="recruitResumeTalentPool_search_form" style="white-space: break-all;word-wrap:break-word;">
					<label style="float:none;white-space:nowrap" >
						<s:text name='recruitResume.recruitDate'/>:
						<input type="text"	id="recruitResumeTalentPool_search_recruit_date_from" name="filter_GED_recruitDate"  style="width:70px;height:15px" class="Wdate"  onclick="WdatePicker({skin:'ext',dateFmt:'yyyy-MM-dd',maxDate:'#F{$dp.$D(\'recruitResumeTalentPool_search_recruit_date_to\')}'})">
						<s:text name='至'/>
					 	<input type="text"	id="recruitResumeTalentPool_search_recruit_date_to"  name="filter_LED_recruitDate" style="width:70px;height:15px" class="Wdate"  onclick="WdatePicker({skin:'ext',dateFmt:'yyyy-MM-dd',minDate:'#F{$dp.$D(\'recruitResumeTalentPool_search_recruit_date_from\')}'})">
					</label>
					<label style="float:none;white-space:nowrap" >
...
<!--      				</label> -->
					...
					<div class="buttonActive" style="float:right">
						<div class="buttonContent">
							<button type="button" onclick="propertyFilterSearch('recruitResumeTalentPool_search_form','recruitResumeTalentPool_gridtable')"><s:text name='button.search'/></button>
						</div>
					</div>
				</form>
			</div>
...
<!-- 			</div> -->
	...
	<div class="pageContent">
		<div class="panelBar" id="recruitResumeTalentPool_buttonBar">
<!-- 			<ul class="toolBar"> -->
<!-- 				<li> -->
<%-- 					<a id="recruitResumeTalentPool_gridtable_del_custom" class="delbutton"  href="javaScript:"><span><s:text name="button.delete" /></span></a> --%>
<!-- 				</li> -->
<!-- 				<li> -->
<!-- 					<a id="recruitResumeTalentPool_gridtable_removeOut_custom" class="exportbutton"  href="javaScript:"><span>移出人才库</span></a> -->
<!-- 				</li> -->
<!-- 				<li> -->
<!-- 					<a id="recruitResumeTalentPool_gridtable_qualified_custom" class="checkbutton"  href="javaScript:"><span>审查合格</span></a> -->
<!-- 				</li> -->
<!-- 				<li> -->
<%--      				<a class="settlebutton"  href="javaScript:setColShow('recruitResumeTalentPool_gridtable','com.huge.ihos.hr.recruitResume.model.RecruitResumeTalentPool')"><span><s:text name="button.setColShow" /></span></a> --%>
<!--     			</li> -->
<!-- 			</ul> -->
		</div>
		<div id="recruitResumeTalentPool_gridtable_div" class="grid-wrapdiv" class="unitBox"
			style="margin-left: 2px; margin-top: 2px; overflow: hidden">
			<input type="hidden" id="recruitResumeTalentPool_gridtable_navTabId" value="${sessionScope.navTabId}">
			<div id="load_recruitResumeTalentPool_gridtable" class='loading ui-state-default ui-state-active' style="display:none"></div>
 			<table id="recruitResumeTalentPool_gridtable"></table>
		</div>
	</div>
	<div class="panelBar" id="recruitResumeTalentPool_pageBar">
		<div class="pages">
			<span><s:text name="pager.perPage" /></span> <select id="recruitResumeTalentPool_gridtable_numPerPage">
				<option value="20">20</option>
				<option value="50">50</option>
				<option value="100">100</option>
				<option value="200">200</option>
			</select> <span><s:text name="pager.items" />. <s:text name="pager.total" /><label id="recruitResumeTalentPool_gridtable_totals"></label><s:text name="pager.items" /></span>
		</div>

		<div id="recruitResumeTalentPool_gridtable_pagination" class="pagination"
