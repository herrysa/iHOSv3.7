...
var navTab = {
	...
	_switchTab: function(iTabIndex){
		...
		//this._getPanels().css({'width':'1px','height':'1px'});
		//this._getPanels().eq(iTabIndex).show();
		this._getPanels().each(function(i){
			if($(this).attr('hideType')=='1'){
				if(i==iTabIndex){
					$(this).css("width","");
					$(this).css("height","");
				}else{
	...
	openTab: function(tabid, url, options){ //if found tabid replace tab, else create a new tab.
		...
		if (iOpenIndex >= 0){
			...
			this._panelBox.append('<div hideType='+op.hideType+' class="page unitBox"></div>');
