...
$.setRegional("alertMsg", {
	title:{error:"Error", info:"Information", warn:"Warning", correct:"Successful",correctUnclose:"Successful", confirm:"Confirmation"},
...
var alertMsg = {
	...
	_types: {error:"error", info:"info", warn:"warn", correct:"correct",correctUnclose:"correctUnclose", confirm:"confirm"},
	...
	_open: function(type, msg, buttons){
		...
		var classType = "";
		if(this._types.correctUnclose == type){
			classType = this._types.correct;
		}else{
			classType = type;
		}
		var boxHtml = DWZ.frag["alertBoxFrag"].replace("#type#", classType).replace("#title#", this._getTitle(classType)).replace("#message#", msg).replace("#butFragment#", butsHtml);
		//$(boxHtml).appendTo("body").css({top:-$(this._boxId).height()+"px"}).animate({top:"0px"}, 500);
		$(boxHtml).appendTo("body").css({top:-$(this._boxId).height()+"px"}).animate({top:jQuery("#container").height()/2+"px"}, {speed:500,step:function(){bgiframe($(this));}});
		/*$(this._boxId).resize(function(e,w,h,t,l,hide){
			bgiframe($(this),e,w,h,t,l,hide);
		});*/
	...
	close: function(){
		$(this._boxId).animate({top:-$(this._boxId).height()}, {speed:500,step:function(){bgiframe($(this));} ,callback:function(){
			$(this).remove();
			bgiframe($(this),null,null,null,null,null,'del');
		}});
		$(this._bgId).hide();
	},
	...
	correctUnclose: function(msg, options) {
		this._alert(this._types.correctUnclose, msg, options);
	},
